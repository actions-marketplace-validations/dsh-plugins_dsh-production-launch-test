...
describe('mergeDshVersions', () => {
  it('合并 npm 与 GitHub Releases，过滤下限，源码版本标 artifact 源', () => {
    ...
    const entries = mergeDshVersions(npm, tags, '0.1.0-rc.6')
    assert.equal(entries.length, 12)
    assert.deepEqual(entries.map(e => e.version), [
    ...
    assert.equal(entries.find(e => e.version === '0.1.2-alpha.1').source, 'artifact:dsh-src-0.1.2-alpha.1')
    assert.equal(entries.find(e => e.version === '0.1.3-alpha.1').source, 'artifact:dsh-src-0.1.3-alpha.1')
    assert.equal(entries.find(e => e.version === '0.1.2-rc.1').source, '')
  })

  it('低于下限的版本被过滤', () => {
    const entries = mergeDshVersions(['0.0.1-rc.1', '0.1.0-rc.6'], [], '0.1.0-rc.6')
    assert.deepEqual(entries.map(e => e.version), ['0.1.0-rc.6'])
  })

  it('非 dsh-v 前缀 tag 与非法版本忽略', () => {
    const entries = mergeDshVersions([], ['launcher-v1.0.0', 'dsh-vx.y.z', 'dsh-v0.1.3-alpha.1'], '0.1.0-rc.6')
    assert.deepEqual(entries.map(e => e.version), ['0.1.3-alpha.1'])
  })
})
